...
namespace Shaman.MM.Tests
{
    ...
    public class RoomManagerTests
    {
        ...
        public async Task GetRoomTest()
        {
            ...
            emptyTask.Wait(1000);
            //success
            var room = _roomManager.GetRoom(_group1Id, 1,1);
            Assert.IsNull(room);
            _roomManager.UpdateRoomState(result.RoomId, 1, RoomState.Open, 1);
            room = _roomManager.GetRoom(_group1Id, 1,1);
            Assert.IsNotNull(room);
            //success
            room = _roomManager.GetRoom(_group1Id, 2,1);
            Assert.IsNotNull(room);
            //no such room
            room = _roomManager.GetRoom(_group1Id, 3,1);
            Assert.IsNull(room);
            ...
            room = _roomManager.GetRoom(_group1Id, 3,1);
            Assert.IsNull(room);
            room = _roomManager.GetRoom(Guid.NewGuid(), 3,1);
            Assert.IsNull(room);
        ...
        public async Task JoinNoServersTest()
        {
            ...
            result = await _roomManager.JoinRoom(result.RoomId, players, 1);
            Assert.IsNotNull(result);
            Assert.AreEqual(result.Result, RoomOperationResult.OK);
            Assert.AreEqual(result.Address, "0.0.0.0");
            Assert.AreEqual(result.Port, 7777);
