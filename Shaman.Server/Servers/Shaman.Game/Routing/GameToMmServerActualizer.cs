...
namespace Shaman.Game.Routing
{
    public class GameToMmServerActualizer : IServerActualizer
    {
        ...
        public GameToMmServerActualizer(IRequestSender requestSender, ITaskSchedulerFactory taskSchedulerFactory, IStatisticsProvider statsProvider, IApplicationConfig config, IShamanLogger logger)
        {
        ...
        public async Task Actualize(int peersCount)
        {
            ...
            var response =
                await _requestSender.SendRequest<ActualizeServerOnMatchMakerResponse>(_config.MatchMakerUrl, request);
            if (!response.Success)
