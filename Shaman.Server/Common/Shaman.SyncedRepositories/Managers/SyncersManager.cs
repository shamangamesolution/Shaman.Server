...
namespace Shaman.SyncedRepositories.Managers
{
    public class SyncersManager : ISyncersManager
    {
        ...
        public void AddSyncer<T>(IRepositorySyncer syncer, int checkConfirmationInterval = 1000, int forceSyncThreshold = 20, int queueDepth = 100, int clearQueuesIntervalMs = 1000, int trackIntervalMs = 1000, int sendTimes = 3) where T:ConfirmChangeIdEventBase
        {
            lock (_mutex)
            {
                _syncers.Add(syncer);
                _typeToSyncers.Add(typeof(T), syncer);
                syncer.Start(checkConfirmationInterval, forceSyncThreshold, queueDepth, clearQueuesIntervalMs, trackIntervalMs, sendTimes);
