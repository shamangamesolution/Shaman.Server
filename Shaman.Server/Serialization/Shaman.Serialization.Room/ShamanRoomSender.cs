...
namespace Shaman.Serialization.Room
{
    public class ShamanRoomSender
    {
        ...
        protected int Send(ISerializable message, DeliveryOptions deliveryOptions, Guid peer)
        {
        ...
        protected int SendToAll(ISerializable message, DeliveryOptions deliveryOptions)
        {
            var stream = _shamanStreamPool.Rent(message.GetType());
            try
            {
