...
namespace Shaman.BackEnd.Data.Repositories
{
    ...
    {
        private ITempRepository tempRepo; 
        private DataStorage _storage;
        public PlayerRepository(string dbServer, string dbName, string dbUser, string dbPassword, IShamanLogger logger, ITempRepository tempRepo)
        ...
        private List<PlayerWalletItem> GetWalletItemListFromDataTable(DataTable dt)
        {
            var result = new List<PlayerWalletItem>();
        ...
        public async Task SetStorage(DataStorage storage)
        {
        ...
        public async Task<Player> GetPlayerInfo(int playerId, SerializationRules serializationRules = SerializationRules.AllInfo)
        {
            try
            {
                var sql = $@"SELECT `players`.`id`,
                ...
                if (playerList.Any())
                {
                    ...
                    if ((serializationRules & SerializationRules.AllInfo) == SerializationRules.AllInfo
                        ...
                        player.Wallet = new PlayerWallet {Items = await GetWalletItems(player.Id)};
        ...
        public async Task<Player> CreatePlayer(string guestId)
        {
            try
            {
                ...
                var id = await CreatePlayer(player);
                //assign id
                player.Id = id;
        ...
        public async Task<List<PlayerWalletItem>> GetWalletItems(int playerId)
        {
            try
            {
